...
# Configuration
ITERATIONS = 25
TEST_MESSAGES = [{"role": "user", "content": "Generate a random 5-word sentence."}]
...
def calculate_stats(latencies: List[float], errors: int, total: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "p95_latency": (
            f"{statistics.quantiles(latencies, n=20)[-1]:.4f}s"
            if len(latencies) > 1
            else "N/A"
        ),
        "errors": errors,
...
async def run_stress_test(name: str, race_func, iterations: int):
    ...
    return {"name": name, "total_wall_time": total_time, "stats": stats}
