...
class FeedbackStorageTest:
    ...
    def generate_random_string(self, length: int = None) -> str:
        ...
        )  # Includes upper/lowercase letters, digits, and space
        ...
        try:
            async with session.post(
                ...
                return FeedbackTestResult(
                    ...
                    feedback_id=(
                        response_data.get("id") if response.status == 202 else None
                    ),
                    error=(
                        response_data.get("detail") if response.status != 202 else None
                    ),
                )
    ...
    async def run_batch_test(self, total_records: int = 100) -> Dict:
            ...
            for i in range(0, total_records, self.batch_size):
                ...
                for result in batch_results:
                    if isinstance(result, Exception):
                        print(
                            f"❌ Error in batch request: {result}",
                            file=open("./feedback_storage_errors.log", "a"),
                        )
                        self.results.append(
                            FeedbackTestResult(
                                success=False,
                ...
                successful_in_batch = sum(
                    ...
                    if isinstance(r, FeedbackTestResult) and r.success
                )
                total_successful = sum(1 for r in self.results if r.success)

                print(
                    f"📦 Batch {batches_completed}: {successful_in_batch}/{current_batch_size} successful, "
                    f"took {batch_time:.2f}s (avg {batch_time/current_batch_size:.3f}s per record)"
                )
                print(
                    f"📊 Overall Progress: {len(self.results)}/{total_records} "
        ...
        return {
            ...
            "records_per_second": (
                len(self.results) / overall_time if overall_time > 0 else 0
            ),
        }
    ...
    def analyze_performance(self, test_summary: Dict):
        ...
        print(
            f"   Successful: {test_summary['successful_records']} ({test_summary['successful_records']/test_summary['total_records']:.1%})"
        )
        print(
            f"   Failed: {test_summary['failed_records']} ({test_summary['failed_records']/test_summary['total_records']:.1%})"
        )
...
async def main():
    ...
    parser.add_argument(
        "--records", type=int, default=100, help="Number of feedback records to create"
    )
    parser.add_argument(
        "--batch-size", type=int, default=10, help="Batch size for concurrent requests"
    )
    parser.add_argument(
