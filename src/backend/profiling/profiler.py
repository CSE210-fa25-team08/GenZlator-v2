...
class LoadTester:
        ...
        async with aiohttp.ClientSession() as session:
            while time.time() - start_time < duration_seconds:
                ...
                if request_count % 50 == 0:
                    success_rate = sum(1 for r in self.results[-50:] if r.success) / min(50, len(self.results))
                    avg_response_time = sum(r.response_time for r in self.results[-50:]) / min(50, len(self.results))
                    print(f"📊 Requests: {request_count}, Success Rate: {success_rate:.1%}, Avg Response: {avg_response_time:.3f}s")
    ...
    async def print_results(self):
        ...
        print(f"Successful: {successful_requests} ({successful_requests/total_requests:.1%})")
...
async def main():
    ...
    parser.add_argument("--qps", type=int, default=10, help="Target queries per second (5-40)")
